...
trabmap_custom_fill_to_trinary_tiff <- function(input_folder, output_folder, voxelsize  ){
  ...
  stack_1 <- trabmap_readtiffstack(path=paste(input_folder, folderlist[1], sep=""), cores=1)
  gc()
  dims <- dim(stack_1)
  ...
  stack_2 <- trabmap_readtiffstack(path=paste(input_folder, folderlist[2], sep=""), cores=1)
  gc()
  dims <- dim(stack_2)
